...
const storage = vi.hoisted(() => ({ getRun: vi.fn().mockResolvedValue({ id: "r1", task_results: [] }), getSubmission: vi.fn(), getCompetition: vi.fn(), getTraceBytes: vi.fn().mockResolvedValue(Buffer.from("secret trace")), putRun: vi.fn(), appendRunEvents: vi.fn() }));
