...
export async function GET(r:NextRequest){
  ...
  else if(kind==="traces"){const run=q.get("run_id"),task=q.get("task_id"),name=q.get("name");if(!run||!task||!name)return NextResponse.json({error:"trace identifiers required"},{status:400,headers:h});const bytes=await s.getTraceBytes(run,task,name);if(!bytes)return NextResponse.json({error:"not_found"},{status:404,headers:h});return NextResponse.json({schema_version:OPS_SCHEMA_VERSION,kind,run_id:run,task_id:task,name,content:bytes.toString("utf8")},{headers:h});}
