...
async function validateEmptyObjectBody(request: NextRequest): Promise<400 | 413 | null> {
  ...
  try {
    ...
    return parsed !== null && typeof parsed === "object" && !Array.isArray(parsed) && Object.keys(parsed).length === 0 ? null : 400;
  } catch { return 400; }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const invalidBody = await validateEmptyObjectBody(request);
