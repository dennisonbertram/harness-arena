...
function CompetitionPerformanceTakeaway({
  ...
  if (!competition || !baseline || !record || baseline.tasksPassed <= 0 || record.tasksPassed <= baseline.tasksPassed) {
    return <PromptIntroduction />;
  }
  ...
  if (!Number.isFinite(baselineCostPerTask) || baselineCostPerTask <= 0 || recordCostPerTask >= baselineCostPerTask) {
    return <PromptIntroduction />;
  }
  ...
  return (
    <div role="region" aria-label={summary}>
      <p className="label" style={{ marginBottom: 10 }}>
        ...
        <span style={{ color: competition.status === "live" ? "var(--green-700)" : "var(--gray-700)" }}>
          {titleCase(competition.status)}
        </span>
      </p>
