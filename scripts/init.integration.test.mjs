...
function runInit(...args) {
  return new Promise((resolve) => {
    const child = spawn("./scripts/init.sh", args, { cwd: root, env: { PATH: process.env.PATH, TMPDIR: process.env.TMPDIR, LANG: process.env.LANG } });
...
describe.sequential("init process ownership integration", () => {
  it("makes simultaneous starts converge on one live owned instance and makes rerun/check idempotent", async () => {
    ...
    expect(ready).toMatchObject({ ok: true, pid: first.pid, nonce: first.nonce, seeded: true, writable: true });
