...
function environmentEvidence(value) {
  ...
  if (!source) throw new Error("invalid_environment_evidence");
  const records = source.slice(0, 500).flatMap((item) => {
    if (!item || typeof item !== "object" || typeof item.key !== "string") return [];
    return [{ name: item.key, targets: stringArray(item.target), type: typeof item.type === "string" ? item.type : "unknown" }];
  });
  const present = new Set(records.filter((item) => item.targets.length === 0 || item.targets.includes("production")).map((item) => item.name));
  return { state: "observed", target: "production", records, required_missing: REQUIRED_ENVIRONMENT.filter((name) => !present.has(name)) };
}
