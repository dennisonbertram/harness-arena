...
// These paths are confined and validated at runtime. Mark the actual fs call
// boundary so Turbopack does not interpret a local-storage path as a request
// to package the repository containing it.
const link = (from, to) => nodeLink(/* turbopackIgnore: true */ from, /* turbopackIgnore: true */ to);
const lstat = (path) => nodeLstat(/* turbopackIgnore: true */ path);
const mkdir = (path, options) => nodeMkdir(/* turbopackIgnore: true */ path, options);
const open = (path, ...options) => nodeOpen(/* turbopackIgnore: true */ path, ...options);
const readFile = (path, ...options) => nodeReadFile(/* turbopackIgnore: true */ path, ...options);
const readdir = (path, ...options) => nodeReaddir(/* turbopackIgnore: true */ path, ...options);
const rename = (from, to) => nodeRename(/* turbopackIgnore: true */ from, /* turbopackIgnore: true */ to);
const rm = (path, options) => nodeRm(/* turbopackIgnore: true */ path, options);
const stat = (path) => nodeStat(/* turbopackIgnore: true */ path);
...
export async function acquireDirectoryLock(lockPath, {
  ...
  const absoluteLock = resolve(/* turbopackIgnore: true */ lockPath);
...
async function inspectLegacyOwner(lockPath, staleMs, confinementRoot) {
  ...
  const ownerPath = join(/* turbopackIgnore: true */ lockPath, "owner.json");
