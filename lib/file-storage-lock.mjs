...
export async function assertNoSymlinksInTree(root, { beforeLstat } = {}) {
  const absoluteRoot = resolve(root);
  let info;
  ...
  for (const entry of await readdir(absoluteRoot)) {
    ...
    await beforeLstat?.(child);
    let childInfo;
    try { childInfo = await lstat(child); } catch (error) {
    ...
    if (childInfo.isDirectory()) await assertNoSymlinksInTree(child, { beforeLstat });
  }
}
