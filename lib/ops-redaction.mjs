...
function jsonObjectEnd(value, start) {
  let depth = 0;
  let quote;
...
// prefixes and may embed more than one JSON object. Parse each complete object
...
function isStrictRunnerTimestampPrefix(value) {
  ...
  return !Number.isNaN(timestamp.valueOf()) && timestamp.toISOString() === match[1];
}
