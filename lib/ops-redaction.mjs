...
function redactTextRecord(record, secrets, state) {
  ...
  for (let index = 0; index < record.length;) {
    ...
    try { output += JSON.stringify(redactValue(parsed, secrets, "", state)); }
    catch { return REDACTED; }
    index = end;
    plainStart = end;
  }
...
function redactSerializedJson(text, secrets, state) {
  ...
  try {
    const redacted = redactValue(parsed, secrets, "", state);
...
function defineEntries(output, entries, secrets, state, used) {
  for (const [name, descriptor] of entries) {
    if (!consumeRedactionBudget(state, "propertiesRemaining")) throw new Error("redaction_property_limit");
