...
describe("shouldReap (pure)", () => {
  it("is true for a queued run whose last event is over 10 minutes old", () => {
    const run = makeRun({ status: "queued" });
    const lastEventTs = "2026-07-21T00:00:00.000Z";
    const now = new Date(lastEventTs).getTime() + TEN_MIN_MS + 1000;
    expect(shouldReap(run, lastEventTs, now)).toBe(true);
  });

  it("is true for a running run whose last event is over 10 minutes old", () => {
    const run = makeRun({ status: "running" });
    const lastEventTs = "2026-07-21T00:00:00.000Z";
    const now = new Date(lastEventTs).getTime() + TEN_MIN_MS + 1;
    expect(shouldReap(run, lastEventTs, now)).toBe(true);
  });

  it("is false when the last event is under 10 minutes old", () => {
    const run = makeRun({ status: "running" });
    const lastEventTs = "2026-07-21T00:00:00.000Z";
    const now = new Date(lastEventTs).getTime() + TEN_MIN_MS - 1000;
...
describe("reapIfStale (integration against MemoryStorage)", () => {
  it("marks a stale queued run reaped and appends a run.reaped event", async () => {
    ...
    const now = new Date("2026-07-21T00:11:00.000Z").getTime();
  ...
  it("uses the timestamp of the most recent event, not created_at, once events exist", async () => {
    ...
    await storage.appendRunEvents(run.id, [
      { ts: "2026-07-21T00:05:00.000Z", type: "task.started", payload: {} },
    ]);

    // 6 minutes after the last event (11 after created_at) -- still under
    // the 10-minute threshold measured from the last event, so this must
...
describe("reapStaleRuns (sweep all runs)", () => {
  it("reaps every stale queued/running run and leaves fresh/terminal runs alone", async () => {
    ...
    const fresh = makeRun({ id: "fresh-1", status: "running", created_at: "2026-07-21T00:09:00.000Z" });
