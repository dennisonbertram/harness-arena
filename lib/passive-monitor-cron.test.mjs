...
describe("Development-only passive Vercel Cron", () => {
  ...
  it("enforces one route-wide deadline across both collectors", async () => {
    ...
    const collectStatus = vi.fn(({ baseUrl, fetchImpl: probeFetch }) => probeFetch(`${baseUrl}/api/health`, { method: "GET", redirect: "manual" }));
    const started = Date.now();
    const result = await executePassiveMonitorCron({ request: request(), env: runtimeEnv(), fetchImpl, collectStatus, now: NOW, deadlineMs: 20 });
