...
pass rate is a mean over a fixed sample instead of a single noisy run. The runs
start under a global concurrency cap, so they may sit `queued` briefly and
finish over time rather than all at once. The submit response returns all 5 run
ids (`run_ids`), plus `run_id` (the first) for convenience.
